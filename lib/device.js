  ...
  isPlaying(callback) {
    this._tryConnect((err) => {
      ...
      this.client.getSessions((err, sessions) => {
        ...
        var promises = sessions.map((session) => {
          return new Promise((resolve, reject) => {
            try {
              this.client.join(session, DefaultMediaReceiver, (e, app) => {
                app.getStatus((e, status) => {
                  if (status && (status.playerState == "PLAYING" || status.playerState == "BUFFERING")) {
                    reject();
                  } else {
                    resolve();
                  }
